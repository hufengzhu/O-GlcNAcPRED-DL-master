...
from O_gly.word2vec.w2v_fea import w2v_fea_hm
...
## ============================================= 数据 ========================================
file = './test_data/independent test sets/Ind_H.fasta'
...
## 5 w2v
w2v_mod = './feature/word2vec/w2v_hm_w4_v30.model'
X2 = w2v_fea_hm(file, w2v_mod).reshape(X.shape[0], 28, 30)


## ============================================= 模型 ========================================
M1 = createModel3()
M1.load_weights('./model/HM/model1.h5')
y_predict_test1 = M1.predict({'word_input': X1})

M2 = createModel3()
M2.load_weights('./model/HM/model2.h5')
y_predict_test2 = M2.predict({'word_input': X1})

M3 = createModel3()
M3.load_weights('./model/HM/model3.h5')
y_predict_test3 = M3.predict({'word_input': X1})

M4 = createModel3()
M4.load_weights('./model/HM/model4.h5')
y_predict_test4 = M4.predict({'word_input': X1})

M5 = createModel1()
M5.load_weights('./model/HM/model5.h5')
...
for index, item in enumerate(y_predict_test1):
    ...
    sum_lst.append(((item*0.05) + (y_predict_test2[index])*0.15 + (y_predict_test3[index])*0.3 + (y_predict_test4[index])*0.2+ (y_predict_test5[index])*0.3))
